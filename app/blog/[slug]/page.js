...
const PostPage = props => {
  ...
  const markdownOptions = {
    overrides: {
      h1: {
        component: ({ children }) => (
          <h1 className="text-3xl text-pink-500 font-bold">{children}</h1>
        ),
      },
      h2: {
        component: ({ children }) => (
          <h2 className="text-2xl text-pink-400 font-bold">{children}</h2>
        ),
      },
      h3: {
        component: ({ children }) => (
          <h3 className="text-xl text-pink-300 font-bold">{children}</h3>
        ),
      },
      a: {
        component: ({ children, href }) => (
          <a href={href} className="text-pink-500 hover:text-pink-700">
            {children}
          </a>
        ),
      },
      strong: {
        component: ({ children }) => (
          <strong className="font-bold text-pink-300">{children}</strong>
        ),
      },
      code: {
        component: ({ children }) => (
